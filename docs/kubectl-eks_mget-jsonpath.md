## kubectl-eks mget-jsonpath

Extract JSONPath values from resources across multiple clusters

### Synopsis

Extract values using JSONPath expressions from Kubernetes resources across clusters that match filters. Example: kubectl eks mget-jsonpath pod my-pod '{.spec.dnsPolicy}' or kubectl eks mget-jsonpath pods '{.spec.dnsPolicy}' to query all pods

```
kubectl-eks mget-jsonpath [resource-type] [resource-name] [jsonpath-expression] [flags]
```
